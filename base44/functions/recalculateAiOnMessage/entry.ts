/**
 * Triggered when a new ChatLog (user message) is created.
 * Recalculates AI metrics for the associated Attempt.
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // Get attempt ID from the ChatLog entity event payload
    const attemptId = body?.data?.attempt;
    if (!attemptId) return Response.json({ skipped: 'no attempt in ChatLog' });
